...
package com.example.graduationproject.clusterutil.clustering.view;
...
import com.example.graduationproject.clusterutil.MarkerManager;
import com.example.graduationproject.clusterutil.clustering.Cluster;
import com.example.graduationproject.clusterutil.clustering.ClusterItem;
import com.example.graduationproject.clusterutil.clustering.ClusterManager;
import com.example.graduationproject.clusterutil.projection.Point;
import com.example.graduationproject.clusterutil.projection.SphericalMercatorProjection;
import com.example.graduationproject.clusterutil.ui.IconGenerator;
...
import static com.example.graduationproject.clusterutil.clustering.algo.NonHierarchicalDistanceBasedAlgorithm.MAX_DISTANCE_AT_ZOOM;
...
public class DefaultClusterRenderer<T extends ClusterItem>
		implements com.example.graduationproject.clusterutil.clustering.view.ClusterRenderer<T> {
	...
	private com.example.graduationproject.clusterutil.ui.SquareTextView makeSquareTextView(Context context) {
		com.example.graduationproject.clusterutil.ui.SquareTextView squareTextView = new com.example.graduationproject.clusterutil.ui.SquareTextView(context);
	 ...
	 * points within the cluster, bucketed to some set points.
	 */
	...
	/**
	 * ViewModifier ensures only one re-rendering of the view occurs at a time, and
	 * schedules re-rendering, which is performed by the RenderTask.
	 */
	...
	private class ViewModifier extends Handler {
		...
		public void queue(Set<? extends Cluster<T>> clusters) {
			synchronized (this) {
				// Overwrite any pending cluster tasks - we don't care about intermediate
				// states.
	...
	/**
	 * Determine whether the cluster should be rendered as individual markers or a
	 * cluster.
	 */
	...
	/**
	 * Transforms the current view (represented by DefaultClusterRenderer.mClusters
	 * and DefaultClusterRenderer.mZoom) to a new zoom level and set of clusters.
	 * <p/>
	 * This must be run off the UI thread. Work is coordinated in the RenderTask,
	 * then queued up to be executed by a MarkerModifier.
	 * <p/>
	 ...
	 * <p/>
	 * When zooming in, markers are animated out from the nearest existing cluster.
	 * When zooming out, existing clusters are animated to the nearest new cluster.
	 */
	private class RenderTask implements Runnable {
		...
		public void run() {
			...
			// Find all of the existing clusters that are on-screen. These are candidates
			// for
			...
			// Don't remove any markers that were just added. This is basically anything
			// that had
			// a hit in the MarkerCache.
			markersToRemove.removeAll(newMarkers);

			// Find all of the new clusters that were added on-screen. These are candidates
			// for
			// markers to animate from.
	...
	private class MarkerModifier extends Handler implements MessageQueue.IdleHandler {
		...
		/**
		 * Animates a markerWithPosition some time in the future, and removes it when
	...
	protected void onBeforeClusterRendered(Cluster<T> cluster, MarkerOptions markerOptions) {
		...
		// TODO: consider adding anchor(.5, .5) (Individual markers will overlap more
		// often)
	...
	private class CreateMarkerTask {
		 ...
		 * @param animateFrom
		 *            the location to animate the markerWithPosition from, or null if no
	...
	/**
	 * A Marker and its position. Marker.getPosition() must be called from the UI
	 * thread, so this object allows lookup from other threads.
	 */
